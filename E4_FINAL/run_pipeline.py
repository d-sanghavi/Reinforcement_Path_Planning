...
def run_pipeline(args):
    ...
    # Extract DXF coords for Vanilla path
    vanilla_dxf_coords = []
    if getattr(planning_result, 'vanilla_path', []):
        vanilla_dxf_coords = [mapper.grid_to_dxf(r, c) for r, c in planning_result.vanilla_path]
    ...
    pdf_path = export_annotated_pdf(
        ...
        vanilla_dxf_coords=vanilla_dxf_coords,
    )
    ...
    metrics_data = {
        ...
        "Baseline DDQN": {
            "Success Rate": getattr(planning_result, 'vanilla_success_rate', 0.0),
            "Average Episode Reward": calc_mean_reward(getattr(planning_result, 'vanilla_episode_rewards', [])),
            "Convergence Speed (Episodes)": getattr(planning_result, 'vanilla_convergence_episode', 0),
            "Path Length (cells)": getattr(planning_result, 'vanilla_path_cost', float("inf")),
            "Path Length in Real World (m)": getattr(planning_result, 'vanilla_path_cost', float("inf")) * real_world_scale if getattr(planning_result, 'vanilla_path_cost', float("inf")) < float("inf") else float("inf"),
            "Planning Time (ms)": getattr(planning_result, 'vanilla_inference_time_ms', 0.0),
            "Total Navigation Time (ms)": getattr(planning_result, 'vanilla_inference_time_ms', 0.0) + planning_result.astar_time_ms,
            "Optimality Ratio": getattr(planning_result, 'vanilla_path_cost', 0) / max(1, planning_result.astar_cost) if planning_result.astar_cost > 0 else 0,
            "Collision Rate": getattr(planning_result, 'vanilla_collisions', 0) / max(1, getattr(planning_result, 'vanilla_path_cost', 1)) if getattr(planning_result, 'vanilla_path_cost', float("inf")) < float("inf") else 0,
            "Dynamic Obstacle Avoidance Rate": 0.0,
            "Path Smoothness": getattr(planning_result, 'vanilla_smoothness', 0.0),
            "Replanning Frequency": 0,
            "CPU Peak Usage (%)": getattr(planning_result, 'vanilla_cpu_peak', 0.0),
            "Memory Peak Usage (MB)": getattr(planning_result, 'vanilla_mem_peak', 0.0)
        }
    }
