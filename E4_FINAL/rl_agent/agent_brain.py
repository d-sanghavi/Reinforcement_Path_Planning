...
class DDQNAgent:
    ...
    # Action index → (delta_row, delta_col). Must stay in sync with env.py ACTIONS.
    _ACTION_DELTAS = [(-1, 0), (1, 0), (0, -1), (0, 1)]  # UP, DOWN, LEFT, RIGHT
    ...
    def select_action(
        ...
        """
        Select an action using epsilon-greedy with Dijkstra-guided exploration.
        ...
        current_pos : tuple, optional
            (row, col) of the agent. Used for Dijkstra-guided exploration.
        dijkstra_map : np.ndarray, optional
            Full 2D Dijkstra distance map. Used to rank neighbour actions.
        grid : np.ndarray, optional
            Occupancy grid (0=free, 1=obstacle). Used to filter wall moves.

        Notes
        -----
        Epsilon decays purely over time (global_step) — NO distance scaling.
        When exploring, the agent prefers the Dijkstra-downhill direction (75%)
        with a 25% chance of any valid non-wall move for variety.
        This prevents both 2-cell oscillation loops and aimless wandering.
        """
        # Pure time-based epsilon — distance ratio scaling is removed.
        if not greedy and random.random() < self.epsilon:
            return self._dijkstra_guided_action(current_pos, dijkstra_map, grid)
    ...
    def _dijkstra_guided_action(self, current_pos, dijkstra_map, grid) -> int:
        """
        Choose an exploratory action guided by the Dijkstra distance map.

        - 75%: pick the valid action that minimises Dijkstra distance (downhill).
        - 25%: pick any valid (non-wall, in-bounds) action at random.
        - Fallback: pure random action when spatial context is unavailable.
        """
        if current_pos is None or dijkstra_map is None or grid is None:
            return random.randint(0, self.action_size - 1)

        r, c = current_pos
        rows, cols = grid.shape

        # Collect valid (non-wall, in-bounds) moves and their Dijkstra costs
        valid_actions = []
        valid_costs   = []
        for action_idx, (dr, dc) in enumerate(self._ACTION_DELTAS):
            nr, nc = r + dr, c + dc
            if 0 <= nr < rows and 0 <= nc < cols and grid[nr, nc] == 0:
                dist = dijkstra_map[nr, nc]
                valid_actions.append(action_idx)
                valid_costs.append(dist if dist != float('inf') else 1e9)

        # Fallback: all neighbours are walls
        if not valid_actions:
            return random.randint(0, self.action_size - 1)

        # 75% greedy downhill; 25% random valid move
        if random.random() < 0.75:
            best_idx = int(np.argmin(valid_costs))
