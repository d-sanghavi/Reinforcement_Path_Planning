...
class LiveTrainingViz:
    ...
    def _build_grid_surface(self, pygame):
        ...
        for r in range(self.rows):
            for c in range(self.cols):
                ...
                # Tint goal region cells light pink/red
                if self.goal_region and (r, c) in self.goal_region:
                    # Don't draw over the exact center goal if it's there
                    if (r, c) != self._goal_pos:
                        rect = pygame.Rect(c * cw, r * ch, cw, ch)
                        color = (255, 100, 100, 100) if self._bg_surface else (255, 200, 200)
                        pygame.draw.rect(surf, color, rect)
    ...
    def _mpl_render(self, positions: dict):
        ...
        try:
            ...
            # Goal Region (Fallback matplotlib)
            if self.goal_region:
                for r, c in self.goal_region:
                    if (r, c) != self._goal_pos:
                        rgb[r, c] = [255, 200, 200]
