...
class GridWorldEnv:
    ...
    def __init__(
        ...
        if self.use_dijkstra_shaping:
            # Add Dijkstra local window and 4 dimensions for Last Action one-hot vector for DDQN
            self.state_size += OBS_WINDOW * OBS_WINDOW + self.action_size
        ...
        # ── Pre-compute Goal Region ──────────────────────────────────────────────
        self.goal_region = set()
        if self.use_dijkstra_shaping and self.dijkstra_map is not None:
            for r in range(self.rows):
                for c in range(self.cols):
                    if self.dijkstra_map[r, c] <= self.goal_radius:
                        self.goal_region.add((r, c))
        else:
            self.goal_region.add(self.goal)
            for r in range(self.rows):
                for c in range(self.cols):
                    if math.hypot(r - self.goal[0], c - self.goal[1]) <= self.goal_radius:
        ...
        logger.debug(
            ...
            f"DijkstraShaping: {use_dijkstra_shaping}, GoalRadius: {self.goal_radius}"
        )
    ...
    def step(self, action: int) -> tuple:
        ...
        # ── Goal check (terminal) ─────────────────────────────────────────────
        if (nr, nc) in self.goal_region:
            # Terminal reward: preserved exactly — no waypoint reward added at goal
            reward = REWARD_GOAL + shaping
            
            # Penalize steps taken by the remaining exact distance to the center goal
            if self.use_dijkstra_shaping and self.dijkstra_map is not None:
                remaining_dist = self.dijkstra_map[nr, nc]
                if remaining_dist != float('inf'):
                    self.steps_taken += int(remaining_dist)
    ...
    def _get_state(self) -> np.ndarray:
        ...
        dijkstra_obs = np.ones((obs_size, obs_size), dtype=np.float32) # default: +1.0 (obstacle/far)

        # Relative Dijkstra normalization: center on the agent's own current distance.
        # This produces values like -0.1 (one step closer) and +0.1 (one step further)
        # instead of the flat global ratio (difference ~0.003) which the network cannot detect.
        agent_dijkstra_dist = 0.0
        if self.use_dijkstra_shaping and self.dijkstra_map is not None:
            d = self.dijkstra_map[r, c]
            agent_dijkstra_dist = d if d != float('inf') else self.max_dijkstra_dist
            ...
            for dc_off in range(-half, half + 1):
                ...
                if 0 <= nr_abs < self.rows and 0 <= nc_abs < self.cols:
                    ...
                    if self.use_dijkstra_shaping and self.dijkstra_map is not None:
                        ...
                        if dist == float('inf'):
                            # Obstacle or unreachable — assign +1.0 (worst/wall sentinel)
                            dijkstra_obs[dr_off + half, dc_off + half] = 1.0
                        else:
                            # Relative slope: negative means closer to goal, positive means further.
                            # Scaled by 10.0 so a 1-step difference = 0.1 (clearly visible to network).
                            relative = (dist - agent_dijkstra_dist) / 10.0
                            dijkstra_obs[dr_off + half, dc_off + half] = float(np.clip(relative, -1.0, 1.0))
        ...
        if self.use_dijkstra_shaping and self.dijkstra_map is not None:
            # DDQN: Includes Dijkstra gradient and Last Action one-hot vector
            last_action_vec = np.zeros(self.action_size, dtype=np.float32)
            if getattr(self, '_last_action', None) is not None:
                last_action_vec[self._last_action] = 1.0
            state = np.concatenate([obs.flatten(), dijkstra_obs.flatten(), [goal_dr, goal_dc], last_action_vec])
        else:
            # PPOA*: Strictly unchanged (observation window + goal direction only)
