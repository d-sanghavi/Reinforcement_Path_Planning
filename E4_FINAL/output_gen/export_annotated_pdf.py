...
def export_annotated_pdf(
    ...
    dpi: int = 200,
    vanilla_dxf_coords: Optional[list] = None,
) -> str:
    """
    ...
    # ── Legend ────────────────────────────────────────────────────────────────
    handles = []
    handles.append(mpatches.Patch(color=path_color, label="Optimal Path"))
    
    if vanilla_dxf_coords and len(vanilla_dxf_coords) >= 2:
        v_xs = [c[0] for c in vanilla_dxf_coords]
        v_ys = [c[1] for c in vanilla_dxf_coords]
        ax.plot(
            v_xs, v_ys,
            color="#FF9800", # Orange
            linewidth=path_linewidth * 0.8,
            linestyle="--",
            solid_capstyle="round",
            solid_joinstyle="round",
            zorder=9,
            label="Baseline DDQN (No Enhancements)",
        )
        handles.append(mpatches.Patch(color="#FF9800", label="Baseline DDQN"))

    handles.append(mpatches.Patch(color="#2dc653", label="Start Point"))
    handles.append(mpatches.Patch(color="#e63946", label="Goal Point"))
