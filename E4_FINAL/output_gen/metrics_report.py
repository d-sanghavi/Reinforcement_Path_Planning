...
def generate_metrics_dashboard(
    ...
    if hasattr(pr, 'vanilla_episode_rewards') and pr.vanilla_episode_rewards:
        eps = list(range(1, len(pr.vanilla_episode_rewards) + 1))
        window = max(1, len(pr.vanilla_episode_rewards) // 10)
        smoothed = np.convolve(pr.vanilla_episode_rewards, np.ones(window) / window, mode="valid")
        ax1.plot(range(window, len(pr.vanilla_episode_rewards) + 1), smoothed, color="#FF9800", linewidth=2, linestyle="--", label="Baseline DDQN")
    ...
    if hasattr(pr, 'vanilla_losses') and pr.vanilla_losses:
        window = max(1, len(pr.vanilla_losses) // 20)
        smoothed = np.convolve(pr.vanilla_losses, np.ones(window)/window, mode="valid")
        ax2.semilogy(range(window, len(pr.vanilla_losses)+1), smoothed, color="#FF9800", linestyle="--", label="Baseline DDQN Loss")
    ...
    if hasattr(pr, 'vanilla_path') and pr.vanilla_path:
        _draw_path_on_grid(ax3, grid, pr.vanilla_path, pr.start, pr.goal, "#FF9800", "Baseline DDQN", overlay=True)
    ...
    vanilla_mean_r = safe_mean(getattr(pr, 'vanilla_episode_rewards', []))
    ...
    vanilla_opt = getattr(pr, 'vanilla_path_cost', float('inf')) / astar_cost
    ...
    vanilla_path_len = len(getattr(pr, 'vanilla_path', []))
    vanilla_cr = (getattr(pr, 'vanilla_collisions', 0) / max(1, vanilla_path_len)) * 100

    table_data = [
        ["Metric",                                  "Baseline",                 "DDQN",                   "PPOA*"],
        ["──────────────────────────────",          "────────",               "────────",               "────────"],
        ["1. Success Rate (Found Goal)",            f"{getattr(pr, 'vanilla_success_rate', 0)*100:.0f}%", f"{getattr(pr, 'ddqn_success_rate', 0)*100:.0f}%", f"{getattr(pr, 'ppoa_success_rate', 0)*100:.0f}%"],
        ["2. Avg Ep Reward (last 10)",              f"{vanilla_mean_r:.1f}",  f"{ddqn_mean_r:.1f}",     f"{ppo_mean_r:.1f}"],
        ["3. Convergence Speed (Ep)",               f"{getattr(pr, 'vanilla_convergence_episode', '-')}", f"{pr.ddqn_convergence_episode}", f"{getattr(pr, 'ppoa_convergence_episode', '-')}"] ,
        ["4. Path Length (Cells)",                  f"{int(getattr(pr, 'vanilla_path_cost', 0)) if getattr(pr, 'vanilla_path_cost', 0)!=float('inf') else '-'}", f"{int(pr.ddqn_path_cost) if pr.ddqn_path_cost!=float('inf') else '-'}", f"{int(getattr(pr, 'ppoa_path_cost', 0)) if getattr(pr, 'ppoa_path_cost', 0)!=float('inf') else '-'}"] ,
        ["5. Planning Time (s)",                    f"{pr.astar_time_ms/1000:.3f}s", f"{pr.astar_time_ms/1000:.3f}s", f"{pr.astar_time_ms/1000:.3f}s"],
        ["6. Navigation Time (s)",                  f"{getattr(pr, 'vanilla_inference_time_ms', 0)/1000:.3f}s", f"{pr.ddqn_inference_time_ms/1000:.3f}s", f"{getattr(pr, 'ppoa_inference_time_ms', 0)/1000:.3f}s"] ,
        ["7. Optimality Ratio",                     f"{vanilla_opt:.2f}",     f"{ddqn_opt:.2f}",        f"{ppo_opt:.2f}"],
        ["8. Collision Rate (%)",                   f"{vanilla_cr:.1f}%",     f"{ddqn_cr:.1f}%",        f"{ppo_cr:.1f}%"],
        ["9. Dyn. Avoidance (%)",                   "-",                      "-",                      f"{getattr(pr, 'ppoa_dyn_avoid_rate', 0.0):.1f}%"],
        ["10. Smoothness (Δ°)",                     f"{getattr(pr, 'vanilla_smoothness', 0):.0f}°", f"{pr.ddqn_smoothness:.0f}°", f"{getattr(pr, 'ppoa_smoothness', 0):.0f}°"],
        ["11. Replanning Freq",                     "-",                      "-",                      f"{getattr(pr, 'ppoa_replans', 0)}x"],
        ["12. CPU Peak (%)",                        f"{getattr(pr, 'vanilla_cpu_peak', 0.0):.1f}%", f"{pr.ddqn_cpu_peak:.1f}%", f"{getattr(pr, 'ppoa_cpu_peak', 0.0):.1f}%"],
        ["13. Memory Peak (MB)",                    f"{getattr(pr, 'vanilla_mem_peak', 0.0):.1f}", f"{pr.ddqn_mem_peak:.1f}", f"{getattr(pr, 'ppoa_mem_peak', 0.0):.1f}"],
    ]
    ...
    for i, row in enumerate(table_data):
        ...
        if i < 2:
            ax4.text(0.02, y, row[0], transform=ax4.transAxes, fontsize=10, color=color_key, fontweight=weight, va="top")
            ax4.text(0.48, y, row[1], transform=ax4.transAxes, fontsize=10, color=color_key, fontweight=weight, va="top")
            ax4.text(0.66, y, row[2], transform=ax4.transAxes, fontsize=10, color=color_key, fontweight=weight, va="top")
            ax4.text(0.84, y, row[3], transform=ax4.transAxes, fontsize=10, color=color_key, fontweight=weight, va="top")
        else:
            ax4.text(0.02, y, row[0], transform=ax4.transAxes, fontsize=10, color=color_key, fontweight=weight, va="top")
            
            # Baseline DDQN (Orange)
            ax4.text(0.45, y, "--", transform=ax4.transAxes, fontsize=10, color="#808090", fontweight=weight, va="top")
            ax4.text(0.48, y, row[1], transform=ax4.transAxes, fontsize=10, color="#FF9800", fontweight=weight, va="top")
            
            # DDQN (Blue)
            ax4.text(0.63, y, "--", transform=ax4.transAxes, fontsize=10, color="#808090", fontweight=weight, va="top")
            ax4.text(0.66, y, row[2], transform=ax4.transAxes, fontsize=10, color=_BLUE, fontweight=weight, va="top")
            
            # PPOA* (Purple)
            ax4.text(0.81, y, "--", transform=ax4.transAxes, fontsize=10, color="#808090", fontweight=weight, va="top")
            ax4.text(0.84, y, row[3], transform=ax4.transAxes, fontsize=10, color=_PURPLE, fontweight=weight, va="top")
